...
1. Publish monolith schema

```
rover subgraph publish monolithodysseyvoyage-786pu@main \
  --schema ./monolith/schema.graphql \
  --name monolith \
  --routing-url 	https://ceda-223-178-210-120.ngrok-free.app
  --convert
  # Convert is used to convert from supergraph to sub-graph
  rover subgraph publish monolithodysseyvoyage-786pu@main \
--schema ./subgraph-accounts/accounts.graphql \
--name accounts \
--routing-url https://afec-223-178-210-120.ngrok-free.app
```

2. Start application
```
 <!-- Starting Monolith Subgraphs -->
 cd server/monolith
 npm run launch
 npm start
 ngrok start --all
 <!-- Starting Router on local machine -->
 cd ../router
 APOLLO_KEY=service:monolithodysseyvoyage-786pu: APOLLO_GRAPH_REF=monolithodysseyvoyage-786pu@main ./router --config ./config.yaml
 <!-- Starting Client -->
 cd ../../client
 npm start
